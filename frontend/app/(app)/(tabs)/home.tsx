...
export default function HomeScreen() {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      {/* ── Pinned bar: the concise header that never leaves the screen ── */}
      <Animated.View style={[styles.bar, heroEntrance]}>
      ...
      >
        {/* The greeting and profile strength scroll away under the bar, so the
            green shrinks to a single row as soon as you start reading. No
            entrance animation here: a band that slides down on mount opens a
            white seam under the bar it is supposed to be joined to. */}
        <HeaderBand>
...
const styles = StyleSheet.create({
  // The safe area carries the bar colour so the status bar area is green too.
  safe: {
    flex: 1,
  ...
  // The concise header: wordmark and bell, always on screen. Everything else
  // that used to sit on the green lives in the HeaderBand below it and scrolls
  // away, so this is all that is left once the user starts reading.
  bar: {
    backgroundColor: Colors.vouchGreen,
  ...
  // The band runs edge to edge at the top of the body, so the page padding
  // sits on the content below it rather than on the scroll view itself.
  sheetContent: {
    paddingBottom: 44,
  },
  sheetInner: {
